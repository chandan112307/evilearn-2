...
_log_api_docs = get_logger("api.documents")
_log_api_validation = get_logger("api.validation")
_log_api_cognitive = get_logger("api.cognitive_load")
_log_api_thinking = get_logger("api.thinking")
_log_api_stress = get_logger("api.stress_test")
_log_api_feedback = get_logger("api.feedback")
_log_api_history = get_logger("api.history")
...
def health_check():
    ...
    _log_api.info("Health check request received")
...
async def upload_document(file: UploadFile = File(...)):
    ...
    _log_api_docs.info(f"Request received: POST /upload-documents, filename={file.filename}")
    ...
    try:
        ...
        _log_api_docs.perf(f"POST /upload-documents completed in {elapsed_ms:.1f}ms")
        _log_api_docs.info(f"Response: document_id={document_id}, status=ready, pages={len(pages)}, chunks={len(chunks)}")
        return response

    except ValueError as e:
        db.update_document_status(document_id, "failed")
        _log_api_docs.error(f"Document upload failed (ValueError): {e}")
        elapsed_ms = (time.perf_counter() - start_time) * 1000
        _log_api_docs.perf(f"POST /upload-documents failed in {elapsed_ms:.1f}ms")
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        db.update_document_status(document_id, "failed")
        _log_api_docs.error(f"Document upload failed (Exception): {e}")
        elapsed_ms = (time.perf_counter() - start_time) * 1000
        _log_api_docs.perf(f"POST /upload-documents failed in {elapsed_ms:.1f}ms")
...
def list_documents():
    ...
    _log_api_docs.info("Request received: GET /documents")
    documents = db.get_documents()
    _log_api_docs.info(f"Response: {len(documents)} documents returned")
...
def process_input(request: ProcessInputRequest):
    ...
    """
    start_time = time.perf_counter()
    _log_api_validation.info(f"Request received: POST /process-input, input_length={len(request.input_text)} chars")
    ...
    try:
        ...
        elapsed_ms = (time.perf_counter() - start_time) * 1000
        _log_api_validation.perf(f"POST /process-input completed in {elapsed_ms:.1f}ms")
        _log_api_validation.info(f"Response: session_id={session_id}, claims={len(claim_results)}")
        return response

    except ValueError as e:
        _log_api_validation.error(f"POST /process-input failed (ValueError): {e}")
        elapsed_ms = (time.perf_counter() - start_time) * 1000
        _log_api_validation.perf(f"POST /process-input failed in {elapsed_ms:.1f}ms")
        raise HTTPException(status_code=400, detail=str(e))
    except RuntimeError as e:
        _log_api_validation.error(f"POST /process-input failed (RuntimeError): {e}")
        elapsed_ms = (time.perf_counter() - start_time) * 1000
        _log_api_validation.perf(f"POST /process-input failed in {elapsed_ms:.1f}ms")
...
def evaluate_reasoning(request: EvaluateReasoningRequest):
    ...
    """
    start_time = time.perf_counter()
    _log_api_stress.info(f"Request received: POST /evaluate-reasoning, answer_length={len(request.student_answer)} chars")
    try:
        ...
        response = EvaluateReasoningResponse(
        ...
        elapsed_ms = (time.perf_counter() - start_time) * 1000
        _log_api_stress.perf(f"POST /evaluate-reasoning completed in {elapsed_ms:.1f}ms")
        _log_api_stress.info(f"Response: weaknesses={len(weakness_items)}, robustness={robustness.level}")
        return response

    except ValueError as e:
        _log_api_stress.error(f"POST /evaluate-reasoning failed (ValueError): {e}")
        elapsed_ms = (time.perf_counter() - start_time) * 1000
        _log_api_stress.perf(f"POST /evaluate-reasoning failed in {elapsed_ms:.1f}ms")
        raise HTTPException(status_code=400, detail=str(e))
    except RuntimeError as e:
        _log_api_stress.error(f"POST /evaluate-reasoning failed (RuntimeError): {e}")
        elapsed_ms = (time.perf_counter() - start_time) * 1000
        _log_api_stress.perf(f"POST /evaluate-reasoning failed in {elapsed_ms:.1f}ms")
        raise HTTPException(status_code=500, detail=str(e))
...
def simulate_thinking(request: ThinkingSimulationRequest):
    ...
    """
    start_time = time.perf_counter()
    _log_api_thinking.info(f"Request received: POST /simulate-thinking, problem_length={len(request.problem)} chars")
    try:
        ...
        elapsed_ms = (time.perf_counter() - start_time) * 1000
        _log_api_thinking.perf(f"POST /simulate-thinking completed in {elapsed_ms:.1f}ms")
        _log_api_thinking.info(f"Response: graphs={len(reasoning_graphs)}, gaps={len(gap_analysis)}")
        return response

    except ValueError as e:
        _log_api_thinking.error(f"POST /simulate-thinking failed (ValueError): {e}")
        elapsed_ms = (time.perf_counter() - start_time) * 1000
        _log_api_thinking.perf(f"POST /simulate-thinking failed in {elapsed_ms:.1f}ms")
        raise HTTPException(status_code=400, detail=str(e))
    except RuntimeError as e:
        _log_api_thinking.error(f"POST /simulate-thinking failed (RuntimeError): {e}")
        elapsed_ms = (time.perf_counter() - start_time) * 1000
        _log_api_thinking.perf(f"POST /simulate-thinking failed in {elapsed_ms:.1f}ms")
        raise HTTPException(status_code=500, detail=str(e))
...
def optimize_cognitive_load(request: CognitiveLoadRequest):
    ...
    """
    start_time = time.perf_counter()
    _log_api_cognitive.info(f"Request received: POST /optimize-cognitive-load, "
                           f"explanation_length={len(request.explanation)} chars, user_id={request.user_id}")
    try:
        ...
        elapsed_ms = (time.perf_counter() - start_time) * 1000
        _log_api_cognitive.perf(f"POST /optimize-cognitive-load completed in {elapsed_ms:.1f}ms")
        _log_api_cognitive.info(f"Response: load_state={response.load_state}, "
                               f"reasoning_mode={response.reasoning_mode}, "
                               f"adapted_steps={len(adapted_steps)}")
        return response

    except ValueError as e:
        _log_api_cognitive.error(f"POST /optimize-cognitive-load failed (ValueError): {e}")
        elapsed_ms = (time.perf_counter() - start_time) * 1000
        _log_api_cognitive.perf(f"POST /optimize-cognitive-load failed in {elapsed_ms:.1f}ms")
        raise HTTPException(status_code=400, detail=str(e))
    except RuntimeError as e:
        _log_api_cognitive.error(f"POST /optimize-cognitive-load failed (RuntimeError): {e}")
        elapsed_ms = (time.perf_counter() - start_time) * 1000
        _log_api_cognitive.perf(f"POST /optimize-cognitive-load failed in {elapsed_ms:.1f}ms")
        raise HTTPException(status_code=500, detail=str(e))
...
def get_results(session_id: str):
    ...
    _log_api_validation.info(f"Request received: GET /get-results/{session_id}")
...
def submit_feedback(request: FeedbackRequest):
    ...
    _log_api_feedback.info(f"Request received: POST /submit-feedback, claim_id={request.claim_id}, "
                           f"session_id={request.session_id}, decision={request.decision}")
    ...
    _log_api_feedback.info(f"Response: feedback_id={feedback_id}")
...
def edit_claim(request: EditClaimRequest):
    ...
    """
    start_time = time.perf_counter()
    _log_api_validation.info(f"Request received: POST /edit-claim, claim_id={request.claim_id}, "
    ...
    try:
        ...
        elapsed_ms = (time.perf_counter() - start_time) * 1000
        _log_api_validation.perf(f"POST /edit-claim completed in {elapsed_ms:.1f}ms")
        _log_api_validation.info(f"Response: claims={len(claim_results)}")
        return response

    except (ValueError, RuntimeError) as e:
        _log_api_validation.error(f"POST /edit-claim failed: {e}")
        elapsed_ms = (time.perf_counter() - start_time) * 1000
        _log_api_validation.perf(f"POST /edit-claim failed in {elapsed_ms:.1f}ms")
        raise HTTPException(status_code=500, detail=str(e))
...
def get_history():
    ...
    _log_api_history.info("Request received: GET /history")
    ...
    _log_api_history.info(f"Response: {len(sessions)} sessions returned")
